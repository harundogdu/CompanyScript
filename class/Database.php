...
class Database extends PDO
{
    ...
    public function loginControl($username, $password)
    {
        ...
        $sqlQuery = "SELECT * FROM admin WHERE admin_username = ? AND admin_password = ? ";
        ...
        if (isset($_FILES[$file])) {
            if ($_FILES[$file]['size'] > 0 && $_FILES[$file]['size'] <= (1024 * 1024 * 5)) :
                ...
                if (in_array($extension, $extensionArr)) :
                    ...
                    if (move_uploaded_file($_FILES[$file]['tmp_name'], "../images/" . $uploadDir . "/" . $filename)) :
                        return $filename;
                    else :
                        return false;
                    endif;
